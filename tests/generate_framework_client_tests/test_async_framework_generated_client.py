...
from tests.framework_clients.async_test_client import client, config, schemas
...
async def test_simple_request_simple_request_get_raises_exception(
    ...
    # Then
    with pytest.raises(httpx.HTTPStatusError) as raised_exception:
        await client.simple_request_simple_request_get()
    assert isinstance(raised_exception.value, httpx.HTTPStatusError)
...
async def test_query_request_optional_query_get(respx_mock: MockRouter):
    ...
    # NOTE: Framework client includes None query parameters as empty strings
...
async def test_header_request_header_request_get(respx_mock: MockRouter):
    ...
    # When
    headers_model = schemas.HeaderRequestHeaderRequestGetHeaders(x_test=input_header)
    response = await client.header_request_header_request_get(
        headers=headers_model.model_dump(by_alias=True, exclude_unset=True)
    )
    # Then
