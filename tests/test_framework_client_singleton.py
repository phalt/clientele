...
@pytest.mark.respx(base_url=BASE_URL)
def test_context_manager_closes_client(respx_mock: MockRouter) -> None:
    """Test that using Client as a context manager properly closes the client."""
    respx_mock.get("/users/1").mock(return_value=httpx.Response(200, json={"id": 1, "name": "Ada"}))
...
@pytest.mark.respx(base_url=BASE_URL)
@pytest.mark.asyncio
async def test_async_context_manager_closes_client(respx_mock: MockRouter) -> None:
    """Test that using Client as an async context manager properly closes the client."""
    respx_mock.get("/users/1").mock(return_value=httpx.Response(200, json={"id": 1, "name": "Ada"}))
    ...
    # After exiting async context, async client should be closed
    assert client._async_client.is_closed


@pytest.mark.respx(base_url=BASE_URL)
def test_context_manager_does_not_close_custom_client(respx_mock: MockRouter) -> None:
    """Test that context manager does not close a custom httpx.Client."""
    custom_httpx_client = httpx.Client(base_url=BASE_URL)
    respx_mock.get("/users/1").mock(return_value=httpx.Response(200, json={"id": 1, "name": "Ada"}))
...
@pytest.mark.respx(base_url=BASE_URL)
@pytest.mark.asyncio
async def test_async_context_manager_does_not_close_custom_client(respx_mock: MockRouter) -> None:
    """Test that async context manager does not close a custom httpx.AsyncClient."""
    custom_httpx_async_client = httpx.AsyncClient(base_url=BASE_URL)
    respx_mock.get("/users/1").mock(return_value=httpx.Response(200, json={"id": 1, "name": "Ada"}))
