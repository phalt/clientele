...
def test_execute_sync_operation_success():
    ...
    mock_introspector.operations = {
        "test_op": Mock(method=mock_operation, http_method="GET")
    }
    mock_config = Mock()
...
def test_execute_async_operation_success():
    ...
    mock_introspector = Mock()
    mock_introspector.operations = {
        "async_op": Mock(method=async_operation, http_method="GET")
    }
    mock_config = Mock()
...
def test_execute_operation_error():
    ...
    mock_introspector = Mock()
    mock_introspector.operations = {
        "error_op": Mock(method=error_operation, http_method="POST")
    }
    mock_config = Mock()
...
def test_execute_with_debug_mode():
    ...
    mock_introspector = Mock()
    mock_introspector.operations = {
        "debug_op": Mock(method=mock_operation, http_method="GET")
    }
    mock_config = Mock()
...
def test_execute_with_config_override():
    ...
    session_config = SessionConfig(
        config_overrides={"base_url": "https://override.com"}
    )
    executor = RequestExecutor(mock_introspector, mock_config, session_config)
