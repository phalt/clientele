...
```sh
clientele scaffold-api -f example_openapi_specs/best.json -o my_client/  --regen
```
...
When you regenerate a client with `--regen`, Clientele follows these rules:
...
```sh
clientele scaffold-api -u http://localhost:8000/openapi.json -o my_client/ --regen
```
